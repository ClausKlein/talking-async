...
auto async_wait_for_signal(asio::signal_set& sigset, CompletionToken&& token)
{
  return asio::async_initiate<CompletionToken,
      ...
      [&sigset](auto handler)
      {
        auto cancellation_slot = asio::get_associated_cancellation_slot(
            handler, asio::cancellation_slot());

        if (cancellation_slot.is_connected()) {
          cancellation_slot.assign([&sigset](asio::cancellation_type /*type*/)
                                   { sigset.cancel(); });
        }
        ...
        auto intermediate_handler =
            [handler = std::move(handler)](std::error_code error,
                                           int signo) mutable
        {
          std::string signame;
          switch (signo) {
        ...
        sigset.async_wait(
            asio::bind_executor(executor, std::move(intermediate_handler)));
      },
      token);
}

auto timed_wait_for_signal() -> awaitable<void>
{
